...
//
// An index of six pages, and a page at a time. The form had grown past a
// thousand lines, which is a setting edited by scrolling until something
// familiar appears - and each index row carries what is set on its page, so
// the common question is answered without opening one.
//
...
Column {
  ...
  // Both halves of what closing and opening mean, in one handler because QML
  // takes one per signal per file - a second `onVisibleChanged` would be the
  // same property assigned twice and the file would not load.
  //
  // Closing: the pane is shut by things that never touch the Close button -
  // Escape unwinds to it, `,` toggles it - so the write happens on the way out
  // however "out" came about. Reading its own `visible` is safe here; it is
  // reading a *child's* that is the circular binding to avoid.
  //
  // Opening: back to the index, and any half-typed building forgotten.
  onVisibleChanged: {
    if (!visible) { flushNow(); return }
    page = pageIndex
    draftPlaceId = ""
    draftPlaceName = ""
  }
  ...
  // Which page is on screen. The index is a page like any other, and is the
  // one every opening starts on: coming back to whichever page was open last
  // time is coming back to a question that has already been answered.
  readonly property int pageIndex: 0
  readonly property int pageAccount: 1
  readonly property int pageChats: 2
  readonly property int pageCalendar: 3
  readonly property int pagePresence: 4
  readonly property int pageWorking: 5
  readonly property int pageLook: 6
  property int page: pageIndex
  ...
  // ---------------- the index ----------------
  //
  // Six pages rather than one column of everything. The form had grown past
  // a thousand lines and a screenful was a fraction of it, which is how a
  // setting comes to be edited by scrolling until something familiar appears -
  // and the wifi rules made it plain, being ticked one network at a time.
  //
  // Every row carries what is set on its page, because "is the calendar on?"
  // is the question most visits actually have, and a row that answers it is a
  // page nobody has to open.
  Column {
    width: parent.width
    spacing: Style.spacing.sm
    visible: root.page === root.pageIndex

    SettingsRow {
      width: parent.width
      title: "Account"
      detail: {
        if (!root.service) return "not configured"
        var named = String(root.current("account", "")).trim()
        if (named === "") return "needs an account name"
        if (root.service.signedIn) return String(root.service.view.username || named)
        return named + "  ·  not signed in"
      }
      fg: Color.foreground
      fontFamily: Style.font.family
      onClicked: root.page = root.pageAccount
    }
    ...
    SettingsRow {
      ...
      detail: {
        var parts = [Number(root.current("chats", 25)) + " chats"]
      ...
      fg: Color.foreground
      fontFamily: Style.font.family
      onClicked: root.page = root.pageChats
    }
    ...
    SettingsRow {
      ...
      title: "Calendar"
      detail: {
        if (root.current("calendar", false) !== true) return "off"
        var parts = [String(root.current("calendarView", "week"))]
        if (root.current("calendarWrite", false) === true) parts.push("can answer and book")
        if (root.current("meetingReminders", true) !== false)
          parts.push(Number(root.current("reminderMinutes", 5)) + " min warning")
        return parts.join("  ·  ")
      }
      fg: Color.foreground
      fontFamily: Style.font.family
      onClicked: root.page = root.pageCalendar
    }
    ...
    SettingsRow {
      ...
      title: "Your presence"
      detail: {
        if (root.current("setPresence", false) !== true) return "off"
        return root.current("holdPresence", false) === true
          ? "on  ·  this machine holds a session" : "on"
      }
      fg: Color.foreground
      fontFamily: Style.font.family
      onClicked: root.page = root.pagePresence
    }

    SettingsRow {
      width: parent.width
      title: "Where you are working from"
      detail: {
        if (root.current("setPresence", false) !== true) return "needs your presence, above"
        var rules = root.wifiRules()
        if (rules.length === 0) return "no networks mapped"
        // The one this machine is on, where there is one: it is the rule the
        // person reading this is most likely asking about.
        var rows = Model.wifiRuleRows(rules, root.service ? root.service.buildings : [])
        var here = root.service ? String(root.service.currentSsid || "") : ""
        for (var i = 0; i < rows.length; i++)
          if (rows[i].ssid.toLowerCase() === here.toLowerCase())
            return here + "  →  " + (rows[i].problem !== "" ? rows[i].problem : rows[i].label)
        return rules.length === 1 ? "1 network mapped" : rules.length + " networks mapped"
      }
      fg: Color.foreground
      fontFamily: Style.font.family
      onClicked: root.page = root.pageWorking
    }
    ...
    SettingsRow {
      ...
      fg: Color.foreground
      fontFamily: Style.font.family
      onClicked: root.page = root.pageLook
    }
  }
  ...
  SettingsPage {
    width: parent.width
    title: "Account"
    open: root.page === root.pageAccount
    spacing: Style.spacing.lg
    onBack: root.page = root.pageIndex
  ...
  SettingsPage {
    width: parent.width
    title: "Calendar"
    open: root.page === root.pageCalendar
    spacing: Style.spacing.lg
    onBack: root.page = root.pageIndex
  ...
  SettingsPage {
    width: parent.width
    title: "Your presence"
    open: root.page === root.pagePresence
    spacing: Style.spacing.lg
    onBack: root.page = root.pageIndex
  ...
  SettingsPage {
    width: parent.width
    title: "Where you are working from"
    open: root.page === root.pageWorking
    spacing: Style.spacing.lg
    onBack: root.page = root.pageIndex

    // Two things: whether the picker may name a building, and which network
    // means which one. The second is deliberately not a text field of SSIDs
    // and GUIDs - it is the network you are on now and a list of buildings to
    // point it at, because the one moment you certainly know which building a
    // wifi belongs to is while you are standing in it.
    Toggle {
      width: parent.width
